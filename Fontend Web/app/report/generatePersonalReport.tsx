...
const GeneratePersonalReport = () => {
  ...
  const [includePortfolioComposition, setIncludePortfolioComposition] = useState(true);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setInstruments((instrs || []).map((i: any) => ({ id: i.id, name: i.name })));
      } catch (e) {
        console.error(e);
  ...
  const loadInstrumentDetails = useCallback(async (instrumentId: number) => {
    ...
    try {
      ...
      setReports((reportsData || []).map((r: any) => ({ id: r.id, period: r.period || `Report #${r.id}` })));
      ...
      if (instrument?.financialMetricId) {
        try {
          ...
          setHasMetric(true);
          setSelectedMetricFields(["PE", "PB", "ROE", "DebtToEquity", "DividendYield"]);
        } catch {
          setHasMetric(false);
        }
      } else {
  ...
  useEffect(() => {
    if (selectedInstrumentId != null) loadInstrumentDetails(selectedInstrumentId);
    else {
      setReports([]);
      setSelectedReportIds([]);
      setHasMetric(false);
  ...
  const canGenerate =
    ...
    (includeInstrumentInfo ||
      (includeFinancialMetrics && (!hasMetric || selectedMetricFields.length > 0)) ||
      (includeFinancialReports && (reports.length === 0 || selectedReportIds.length > 0)) ||
  ...
  const handleGenerate = async () => {
    ...
    if (includeFinancialReports && reports.length > 0 && selectedReportIds.length === 0) {
      Alert.alert(
        "Select periods",
    ...
    try {
      await ApiService.generatePersonalReportPdf({
        ...
        includedMetricFields:
          includeFinancialMetrics && selectedMetricFields.length > 0 ? selectedMetricFields : undefined,
        includeFinancialReports,
        includedFinancialReportIds: includeFinancialReports ? selectedReportIds : undefined,
        includePortfolioComposition,
      });
  ...
  return (
    <ScrollView contentContainerStyle={[globalStyles.scrollContainer, spacing.p4]}>
      <Text style={[globalStyles.header, spacing.mb4]}>Generate Personal Report</Text>
      <Text style={[globalStyles.text, spacing.mb4]}>
        Choose an instrument first, then select which data to include in the PDF.
      </Text>

      {/* 1. Instrument dropdown at top */}
      <View style={[globalStyles.card, spacing.p4, spacing.mb4]}>
        <Text style={[globalStyles.label, spacing.mb2]}>Investment instrument</Text>
        <View
          style={[
            globalStyles.pickerWrapper,
            globalStyles.pickerWebWrapper,
            { flexDirection: "row", alignItems: "center", paddingHorizontal: 12, height: 48 },
          ]}
        >
          <Picker
            selectedValue={selectedInstrumentId ?? ""}
            onValueChange={(v) => setSelectedInstrumentId(v ? Number(v) : null)}
      ...
      {selectedInstrumentId != null && (
        <>
          ...
          <View style={[globalStyles.card, spacing.p4, spacing.mb4]}>
            <Text style={[globalStyles.label, spacing.mb2]}>Instrument details</Text>
            <View style={[globalStyles.row, globalStyles.spaceBetween]}>
              <Text style={globalStyles.text}>Include instrument details in report</Text>
              <Switch value={includeInstrumentInfo} onValueChange={setIncludeInstrumentInfo} />
            </View>
          </View>

          {/* 3. Financial metrics – which fields */}
          <View style={[globalStyles.card, spacing.p4, spacing.mb4]}>
            <View style={[globalStyles.row, globalStyles.spaceBetween, spacing.mb2]}>
              <Text style={globalStyles.label}>Financial metrics</Text>
              <Switch value={includeFinancialMetrics} onValueChange={setIncludeFinancialMetrics} />
            </View>
            {includeFinancialMetrics && (
              <>
                ...
                {hasMetric ? (
                  METRIC_FIELDS.map((f) => (
                    <View
                      key={f.key}
                      style={[globalStyles.row, globalStyles.spaceBetween, spacing.py1]}
                    >
                  ...
                  <Text style={globalStyles.textSmall}>No financial metrics for this instrument.</Text>
                )}
              </>
            )}
          </View>
          ...
          <View style={[globalStyles.card, spacing.p4, spacing.mb4]}>
            ...
            {includeFinancialReports && (
              <>
                ...
                {reports.length === 0 ? (
                  <Text style={globalStyles.textSmall}>No financial reports for this instrument.</Text>
                ) : (
                  reports.map((r) => (
                    <View
                      key={r.id}
          ...
          <View style={[globalStyles.card, spacing.p4, spacing.mb4]}>
            <Text style={[globalStyles.label, spacing.mb2]}>Portfolio composition</Text>
            <View style={[globalStyles.row, globalStyles.spaceBetween]}>
              <Text style={globalStyles.text}>Include full wallet (all positions)</Text>
              <Switch
