...
const ApiService = {
  ...
  getWalletInstrumentsByWalletId(walletId) {
    return api
      .get(`/WalletInstrument/wallet/${walletId}`)
      .then((res) => res.data);
  },
  ...
  getWalletInstrumentById(id) {
    return api
      .get(`/WalletInstrument/${id}`)
      .then((res) => res.data);
  },

  updateWalletInstrument(id, data) {
    return api
      .put(`/WalletInstrument/${id}`, data)
      .then((res) => res.data);
  },

  deleteWalletInstrument(id) {
    return api
      .delete(`/WalletInstrument/${id}`)
      .then((res) => res.data);
  },
...
updateWallet(walletId, data) {
  ...
  generateInvestmentRecommendationPdf(analysisRequestId: number) {
    if (Platform.OS === "web") {
      window.open(
        `${api.defaults.baseURL}/InvestmentRecommendationPdf/${analysisRequestId}`,
        "_blank"
      );
    ...
    return AsyncStorage.getItem("userToken").then((token) => {
      ...
      return downloadResumable.downloadAsync().then((res) => res?.uri);
    });
  },

  downloadInvestmentRecommendationPdfToTemp(analysisRequestId: number) {
    return AsyncStorage.getItem("userToken").then((token) => {
      const fileUri =
...
async downloadWalletExcel(walletId: number) {
  if (Platform.OS === "web") {
    window.open(
      `${api.defaults.baseURL}/Wallet/${walletId}/export`,
      "_blank"
    );
    return;
  }
  ...
  const fileUri =
    FileSystem.documentDirectory + `Wallet_${walletId}.xlsx`;
...
async downloadWalletExcelToTemp(walletId: number) {
  ...
  const fileUri =
    FileSystem.cacheDirectory + `Wallet_${walletId}.xlsx`;
