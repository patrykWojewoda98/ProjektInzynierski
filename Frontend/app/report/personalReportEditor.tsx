...
const PersonalReportEditor = () => {
  ...
  const { instrumentId, includeInstrumentInfo, includeFinancialMetrics, includedMetricFields, includeFinancialReports, includedReportIds, includePortfolioComposition } = useMemo(() => {
    const rawId = params.selectedInstrumentId;
    const id = rawId ? parseInt(String(rawId), 10) : null;
    if (id !== null && isNaN(id)) {
      return {
        instrumentId: null as number | null,
        includeInstrumentInfo: false,
        includeFinancialMetrics: false,
        includedMetricFields: [] as string[],
        includeFinancialReports: false,
        includedReportIds: [] as number[],
        includePortfolioComposition: false,
      };
    }
    let metricFields: string[] = [];
    let reportIds: number[] = [];
    try {
      if (params.metricFields) metricFields = JSON.parse(String(params.metricFields));
    } catch {
      // ignore
    }
    try {
      if (params.reportIds) reportIds = JSON.parse(String(params.reportIds));
    } catch {
      // ignore
    }
    return {
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      ...
      if (includeFinancialMetrics && includedMetricFields.length > 0) {
        parts.push(
          `Financial metrics: ${includedMetricFields.join(", ")}`,
        );
      }
  ...
  const handleGenerateAndSave = async () => {
    ...
    try {
      ...
      if (!permissions.granted) {
        Alert.alert(
          "Cancelled",
          "Folder access is needed to save the PDF to your device.",
        );
        return;
      }
      const fileName = `Personal_Report_Editor_${Date.now()}.pdf`;
      ...
      Alert.alert(
        "Saved",
        "PDF saved successfully to the selected folder on your device.",
      );
    } catch (e) {
      console.error(e);
  ...
  return (
    ...
    >
      ...
      <Text style={[globalStyles.text, spacing.mb4]}>
        ...
        text and choose font, then generate the PDF. The file will be saved
